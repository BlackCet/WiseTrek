...
function HomePage() {
  ...
  // ✅ TOP LEVEL HOOK 1: Count categories for the summary UI
  ...
  // ✅ TOP LEVEL HOOK 2: Filter the events based on selected category
  const displayedEvents = useMemo(() => {
    if (!formData.category) return events;
    return events.filter(
      (event) => classifyEvent(event) === formData.category
    );
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const eventRes = await apiService.get("/search-events", {
        params: formData,
      });
      setEvents(eventRes.data);

      const weatherRes = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/weather?city=${formData.destination}`
      );
          ...
          <div className="flex items-center gap-4 animate-in fade-in slide-in-from-right-4">
            <div className="flex items-center gap-3 bg-white pl-1 pr-5 py-1 rounded-full shadow-sm border border-gray-100">
              ...
              <div>
                ...
                <Typography className="font-bold text-gray-800 text-sm leading-none mt-1">
                  {user.name}
                </Typography>
              </div>
            </div>
            <Tooltip title="Logout">
              <IconButton
                onClick={logout}
                className="bg-white hover:bg-red-50 text-gray-400 hover:text-red-500 transition-all border border-gray-100 shadow-sm"
              >
                <LogOut size={18} />
              </IconButton>
            </Tooltip>
      ...
      <Container maxWidth="lg" className="pt-12 pb-24">
        <div className="text-center mb-14 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <p className="text-gray-500 text-lg md:text-xl font-medium max-w-2xl mx-auto leading-relaxed">
            Discover the perfect blend of local events and real-time weather for
            your next adventure.
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
              ...
              <div className="md:col-span-3 group">
                ...
                <div className="bg-gray-50/80 group-hover:bg-purple-50/50 border border-transparent group-hover:border-purple-100 rounded-3xl p-3 transition-all duration-300 flex items-center">
                  <CalendarDays
                    size={20}
                    className="ml-2 mr-3 text-purple-500 shrink-0"
                  />
                  <TextField
                    type="date"
                    fullWidth
              ...
              <div className="md:col-span-3 group">
                ...
                <div className="bg-gray-50/80 group-hover:bg-pink-50/50 border border-transparent group-hover:border-pink-100 rounded-3xl p-3 transition-all duration-300 flex items-center">
                  <CalendarDays
                    size={20}
                    className="ml-2 mr-3 text-pink-500 shrink-0"
                  />
                  <TextField
                    type="date"
                    fullWidth
              ...
              <div className="md:col-span-2 group">
                ...
                <div className="bg-gray-50/80 group-hover:bg-orange-50/50 border border-transparent group-hover:border-orange-100 rounded-3xl p-3 transition-all duration-300 flex items-center">
                  <Compass
                    size={20}
                    className="ml-2 mr-3 text-orange-500 shrink-0"
                  />
                  <TextField
                    select
                    fullWidth
                    ...
                    SelectProps={{
                      ...
                      MenuProps: {
                        PaperProps: {
                          className:
                            "rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-gray-100 mt-2 p-1",
                          elevation: 0,
                        },
                      },
                    }}
                  ...
                  >
                    <MenuItem
                      value=""
                      className="rounded-xl mx-1 my-0.5 text-sm font-medium hover:bg-gray-50"
                    >
                      All Events
                    </MenuItem>
                    <MenuItem
          ...
          <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-5 duration-500">
            ...
            <div className="flex items-center justify-between mb-8 mt-20">
              <div className="flex items-center gap-3">
                ...
                <Typography
                  variant="h5"
                  className="font-bold text-gray-900 text-2xl tracking-tight"
                >
                  Upcoming Events
                </Typography>
              </div>
            ...
            <div className="mt-12 p-8 bg-white rounded-3xl shadow-lg border border-gray-100">
              <div className="grid grid-cols-5 gap-4">
                {Object.entries(categoryCounts).map(([cat, count]) => {
                  ...
                  // ✅ REMOVED the redundant useMemo from here
                  return (
