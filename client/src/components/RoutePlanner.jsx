...
// // SAME conversion as your WORKING MAP CODE
// const toLatLng = (multiLine) =>
//   multiLine[0].map(([lng, lat]) => [lat, lng]);
...
//   const mapData = drive || bus;
//   const center = mapData
//     ? toLatLng(mapData.geometry)[0]
//     : [20.5937, 78.9629];
...
//             {loading ? "Loading..." : "Compare Routes"}
//           </button>
//         </form>
//       </div>
...
//       {/* MAP — SAME STRUCTURE AS WORKING CODE */}
//       {(drive || bus) && (
//         <div style={styles.card}>
//           <MapContainer
//             center={center}
...
//             {drive && (
//               <Polyline
//                 positions={toLatLng(drive.geometry)}
//                 pathOptions={{ color: "#3b2f2f", weight: 4 }}
//               />
//             )}

//             {bus && (
//               <Polyline
//                 positions={toLatLng(bus.geometry)}
//                 pathOptions={{
//                   color: "#F8C794",
//                   weight: 4,
//                   dashArray: "6",
...
const RoutePlanner = () => {
  ...
  // FIXED: Improved Parser to handle your specific Postman response strings
  // const calculatePriceRange = (fares) => {
  //   if (!fares || fares.length === 0) return "Check IRCTC";
    
  //   // Extract all numbers that follow a ₹ symbol from all strings in the array
  //   const allPrices = fares.flatMap(str => {
  //     const matches = str.match(/₹(\d+)/g);
  //     return matches ? matches.map(m => parseInt(m.replace('₹', ''))) : [];
  //   });

  //   if (allPrices.length === 0) return "Pricing Unavailable";
